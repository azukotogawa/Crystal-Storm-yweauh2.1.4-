...
## Session record — 2026-07-15 (P2 core gameplay loop)

**Backlog item addressed:** P2 — make maze→assault→victory loop playable with visible terrain defense, viscous crystal, combat win path, and progression feedback.

### Audit (pre-change)
- Gating probes already PASS: `verify_crystal_terrain_routing`, `verify_crystal_settling`, `verify_crystal_flow_mechanics`, `verify_spawn_goal`, `verify_combat_crystal_damage`, `verify_full_game_loop`.
- Gap: absorption unlocks did not grant relics; `game_overlay` lacked player-visible unlock/relic feedback; `GameManager` loss reason not shown on LOST panel.

### Implemented
- `crystal/crystal_manager.gd` — `relic_for_enemy_unlock()` map (`crystal_mite`→`mason_glove`, `thornling`/`shard_guard`→`flow_anchor`); `_grant_relic_for_unlock()` equips via `_resolve_player_relic_manager()` (Player property or `RelicManager` child) on vegetation and ruin absorption unlocks.
- `ui/game_overlay.gd` + `game_overlay.tscn` — toast line for absorption/unlock/relic/spawn-destroy events; HUD progression line (unlocks + relics); spawn-destroy toast; LOST panel shows `GameManager.last_loss_reason`; progression binding retries until `evolution` + `RelicManager` are wired (not marked bound early).
- `game/game_manager.gd` — `last_loss_reason` stored on `_set_lost`.
- `weapons/weapon_controller.gd` — `_crystal_target_cell` uses attack-mode column targeting (not `&"any"` / mouse-only).
- `scripts/verify_p2_progression_feedback.gd` — E2E: evolution unlock → `_grant_relic_for_unlock` → equipped relic + overlay HUD/toast (suite 95).
- `scripts/verify_combat_crystal_damage.gd` — honest fail path; main-scene production `_do_melee_attack` damages spawn; `ProbeExit` + abrupt OK marker in runner.

### Verified (automated)
- `verify_p2_progression_feedback.gd`: PASS.
- P2 gating suite (terrain routing, settling, flow, spawn goal, combat damage, full loop): PASS (pre-change audit).
- Full suite: **95/95** `run_all_verify.sh` PASS (`/tmp/grok-goal-3b50c9e7cdd3/implementer/p2_verify_all.log`).
- Targeted logs: `p2_verify_combat_crystal_damage.log`, `p2_verify_progression_feedback.log` (honest PASS/FAIL, production melee HP drop).

### Human playtest (T key / dev tools)
1. **T** → dev chat; use scenario preset `builder` or `dig_flat` — dig a channel toward crystal; confirm spread pools in the trench.
2. Build wall choke with stone (RMB) — crystal should slow at the column (`BUILD_FLOW_BLOCK` stat).
3. Let crystal absorb grass (10 cells) — toast + HUD should show unlock and **Mason's Glove** relic.
4. Absorb bushes (6) — **Flow Anchor** relic toast; walls resist spread more.
5. Assault: destroy minor spawns with sword; HUD shows `N/M spawns`; origin boss sealed until minors cleared.
6. Win: destroy all spawns → victory overlay; **R** restarts. Lose: let coverage exceed HUD `Crystal X% / Y%` limit → LOST panel with reason.

### Deferred
- Economy/crafting trees, boss phases, expanded relic roster, town consequence tuning (per P2 non-goals).
- Human fun/feel sign-off — not auto-written to `manual_verification.md`.

## Session record — 2026-07-15 (runtime performance instrumentation)

**Backlog item addressed:** Lightweight runtime profiler in F3 debug overlay (measurement only, no optimization).

### Implemented
- `systems/perf_profiler.gd` — `get_runtime_report()`, `format_runtime_report()`, `inc_rate`/`inc_frame`, `sample_scene_stats()`; six OBJECTIVE sections (FRAME/CHUNKS/CRYSTAL/ENTITIES/RENDER/WORLD).
- Instrumentation: `chunk_manager` (generated/rebuilt rates, `get_render_stats`), `crystal_manager` (`get_profiler_stats`, frontier gauge), `crystal_terrain_query` (terrain query frame counter), `world_entity`/`crystal_enemy` (`entity_physics` timing).
- `ui/debug_panel.gd` — throttled `sample_scene_stats` + appended structured perf block on F3 overlay (preserves existing debug lines).
- `scripts/verify_runtime_profiler.gd`, `scripts/verify_profiler_main.gd` (suites 96–97).

### Learned
- Draw calls via `RenderingServer.RENDERING_INFO_TOTAL_DRAW_CALLS_IN_FRAME` on overlay refresh cadence (not per-frame tree walk).
- Section timings show **last-frame** `last_us` values via `_last_sections` snapshot (taken before idle-frame zeroing so `_physics_process` entity timings survive).
- MultiMesh/instance totals sampled from loaded `ChunkView` layers only when overlay refreshes.

### Verified
- `verify_runtime_profiler.gd`, `verify_profiler_main.gd`: PASS (`/tmp/grok-goal-59b157a7ebbb/implementer/`).
- Full suite: **97/97** `run_all_verify.sh` PASS (`/tmp/grok-goal-59b157a7ebbb/implementer/verify_all.log`).
- `verify_main_runtime_health.gd`: PASS (`profiler_main_boot.log`).

### Human check (F3)
- Press **F3** in main scene — scroll debug overlay for FRAME/CHUNKS/CRYSTAL/ENTITIES/RENDER/WORLD blocks; values should update while moving, digging, and during crystal spread.
