...
func _run() -> void:
	...
	if "update_dirty_column_maps" not in cm_src or "invalidate_columns_at_world" not in cm_src:
		push_error("chunk rebuild must use incremental dirty-column path for terrain edits")
		failed = true
	elif "_build_mesh_region" not in cm_src:
		push_error("chunk rebuild must patch mesh regions for terrain edits")
		failed = true
	else:
		print("OK chunk rebuild path is incremental column-map + mesh patch")
	...
	for cell in solid_cells:
		...
		var dig_info: Dictionary = _ActionTargeting.resolve_action(
			player, world, chunk_manager, pick_range, false
		)
