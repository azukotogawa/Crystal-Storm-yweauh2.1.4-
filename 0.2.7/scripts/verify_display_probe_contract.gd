...
func _run() -> void:
	...
	if 'Input.action_press("ui_right")' not in text:
		...
		ok = false
	if "human visual confirm still pending" not in text and "data only; visual confirm pending" not in text:
		push_error("probe dig line must state human visual pending (not visible carve claim)")
		ok = false
	if "chunk mesh surface_y=" not in text:
		push_error("probe dig must corroborate chunk mesh surface_y after rebuild")
		ok = false
