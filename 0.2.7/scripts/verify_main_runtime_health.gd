...
func _run() -> void:
	...
	for coord in chunk_manager.chunks.keys():
		...
		if mm != null and mm.multimesh != null and mm.multimesh.instance_count > 0:
			terrain_mat = mm.material_override as ShaderMaterial
		else:
			var surf: MeshInstance3D = view.get_node_or_null("LayerContainer/terrain_surface_mesh") as MeshInstance3D
			if surf != null and surf.mesh != null and surf.mesh.get_surface_count() > 0:
				terrain_mat = surf.material_override as ShaderMaterial
		if terrain_mat == null:
			continue
		instances_checked += 1
		var bound: Texture2D = terrain_mat.get_shader_parameter("texture_atlas") as Texture2D
		if bound == null:
	...
	if not atlas_ok:
		push_error("no chunk terrain mesh with bound Cube.png atlas")
		failed = true
