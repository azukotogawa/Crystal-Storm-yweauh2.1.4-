...
func _run() -> void:
	...
	for coord in chunk_manager.chunks.keys():
		...
		if mm and mm.multimesh != null and mm.multimesh.instance_count > 0:
			...
			if surf and surf.mesh != null and surf.mesh.get_surface_count() > 0:
				terrain_mat = surf.material_override as ShaderMaterial
		if terrain_mat:
			var tex: Texture2D = terrain_mat.get_shader_parameter("texture_atlas") as Texture2D
