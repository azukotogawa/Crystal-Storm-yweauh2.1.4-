...
# Living World density (towns/villagers/ruins) pushes boot + probe past 90s on modest GPUs.
timeout 150s env CRYSTALSTORM_PERF_PRESET=medium CRYSTALSTORM_PROBE_ABRUPT_EXIT=1 CRYSTALSTORM_SCRATCH="$SCRATCH_DIR/display_session_evidence.md" godot --path "$ROOT" -s "$ROOT/scripts/display_session_probe.gd" 2>&1 | tee "$LOG" || true
