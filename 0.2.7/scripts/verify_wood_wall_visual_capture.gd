extends SceneTree
## Boot main.tscn, place wood_wall arrangements, capture gameplay-camera screenshots.
## Usage: godot --path . -s scripts/verify_wood_wall_visual_capture.gd
...
const OUT_DIR := "/tmp/wood_wall_capture"
...
func _run() -> void:
	...
	# Beside terrain (dug cell)
	var dig_cell := Vector2i(px + 9, pz)
	_TerrainEdits.dig(dig_cell.x, dig_cell.y, 1)
	var by_terrain := Vector2i(px + 10, pz)
	# Gate + adjacent wall
	var gate_cell := Vector2i(px + 5, pz + 3)
	var by_gate := Vector2i(px + 6, pz + 3)
	# Chunk boundary (SIZE=16): 14,15 | 16,17
	var boundary: Array[Vector2i] = [
		Vector2i(14, pz + 8), Vector2i(15, pz + 8), Vector2i(16, pz + 8), Vector2i(17, pz + 8)
	]

	var all_walls: Array[Vector2i] = [isolated, by_terrain, by_gate]
	all_walls.append_array(line_x)
	all_walls.append_array(corner)
	all_walls.append_array(boundary)
	...
	for c in all_walls:
		...
		if not editor.try_build_wall(Vector3(float(c.x) + 0.5, y, float(c.y) + 0.5), inv, false):
			_fail("place %s: %s" % [str(c), editor.last_fail_reason])
	...
	# Let meshes apply + chunk rebuild from raised terrain.
	for _j in 45:
		await process_frame
	...
	for c in all_walls:
		...
		if str(mesh.get_meta("authored_resource_path", "")) != WOOD_MESH_PATH:
			_fail("path %s" % mesh.get_meta("authored_resource_path", ""))
			continue
		var vs: float = _WorldSettings.get_active().voxel_scale
		if not mesh.scale.is_equal_approx(Vector3(vs, vs, vs)):
			_fail("scale %s expected %s" % [str(mesh.scale), str(Vector3(vs, vs, vs))])
		authored += 1

	_ok("authored walls=%d / %d" % [authored, all_walls.size()])
	if authored < all_walls.size():
		_fail("missing authored walls")

	# No duplicate anchors per cell
	var seen: Dictionary = {}
	for key in feat_layer._nodes_by_cell.keys():
		var node: Node = feat_layer._nodes_by_cell[key]
		if node == null:
			continue
		if seen.has(node):
			_fail("duplicate node for cells")
		seen[node] = key
	_ok("no duplicate structure nodes in cell map")

	var ga: Node3D = feat_layer._nodes_by_cell.get(gate_cell)
	if ga == null or str(ga.get_meta("building_visual_id", "")) != "gate":
		_fail("gate identity lost")
	else:
		_ok("gate remains gate beside wall")

	# Overview screenshot
	_frame_camera(player, cam, world, px + 5, pz + 2)
	await _capture("overview")

	# Isolated close-up
	_frame_camera(player, cam, world, isolated.x, isolated.y, 10.0)
	await _capture("isolated")

	# Line
	_frame_camera(player, cam, world, px + 5, pz, 12.0)
	await _capture("line")
	...
	if not editor.try_build_wall(Vector3(float(mid.x) + 0.5, y2, float(mid.y) + 0.5), inv, false):
	...
	if rebuilt == null or str(rebuilt.get_meta("building_visual_id", "")) != "wood_wall":
		_fail("rebuild lost wood_wall")
	else:
...
func _finish() -> void:
	if _failed == 0:
		_ProbeExit.finish_tree(self, 0, "WOOD WALL VISUAL CAPTURE OK")
	else:
		_ProbeExit.finish_tree(self, 1, "WOOD WALL VISUAL CAPTURE FAILED")
