...
func _bootstrap_when_ready() -> void:
	...
	if _STP and _STP.is_enabled():
		_STP.begin("CrystalManager.bootstrap_when_ready", "synchronous_main")
	...
	_initialized = true
	if _STP and _STP.is_enabled():
		_STP.end("CrystalManager.bootstrap_when_ready", {
			"bootstrap_ms": Time.get_ticks_msec() - t_boot0,
			"spawns": _spawn_points.size() if _spawn_points else 0,
		})
	if _crystal_measure_enabled():
...
func _on_player_terrain_edited(wx: int, wz: int, _kind: StringName) -> void:
	if _terrain_query and _terrain_query.has_method("invalidate_terrain_caches"):
		_terrain_query.invalidate_terrain_caches()
	# Floor heights change with dig/build — rebuild nearby crystal meshes so clusters
	# do not stay embedded in old terrain (presentation only; sim depth unchanged).
	_refresh_crystal_floor_near(wx, wz)


func _refresh_crystal_floor_near(wx: int, wz: int, radius: int = 2) -> void:
	if _presentation == null or _sim == null:
		return
	var min_d: float = sim_config.min_depth if sim_config else 0.04
	var dirty_pos: Array = []
	for dx in range(-radius, radius + 1):
		for dz in range(-radius, radius + 1):
			var pos := Vector2i(wx + dx, wz + dz)
			if float(_sim.depth.get(pos, 0.0)) < min_d:
				continue
			dirty_pos.append(pos)
	if dirty_pos.is_empty():
		return
	# Force full chunk rebuild so terrain_y is re-sampled (patch alone can keep old floor).
	for pos_v in dirty_pos:
		var pos2: Vector2i = pos_v
		var coord := Vector2i(
			floori(float(pos2.x) / float(ChunkData.SIZE)),
			floori(float(pos2.y) / float(ChunkData.SIZE))
		)
		if "_chunk_needs_full_rebuild" in _presentation:
			_presentation._chunk_needs_full_rebuild[coord] = true
	_presentation.apply_events([_CrystalSimEvents.mesh_dirty(dirty_pos)])
...
func get_origin_cell() -> Vector2i:
	if _spawn_points.size() > 0:
		return _spawn_points[0].world_pos
	# Map-center threat before spawn init. Relocated land (e.g. (−10,10))
	# still sits in chunk (−1,0), covered by origin ±1 stream extras.
...
func _bind_chunk_stream() -> void:
	...
	# Origin extras may already be resident before this bind — chunk_ready will not re-fire.
	if _presentation and _presentation.has_method("mark_all_indexed_dirty"):
...
func _refresh_spawn_markers() -> void:
	...
	for spawn in _spawn_points:
		...
		if not has_tex:
			...
			prism.name = "FallbackPrism"
			var box := BoxMesh.new()
			box.size = Vector3(marker_size, marker_size * 1.4, marker_size)
			prism.mesh = box
			var pmat := StandardMaterial3D.new()
			pmat.albedo_color = Color(0.78, 0.28, 1.0, 0.92)
			pmat.emission_enabled = true
			pmat.emission = Color(0.55, 0.18, 0.95)
			pmat.emission_energy_multiplier = 2.0
			prism.material_override = pmat
			prism.position.y = marker_size * 0.15
			anchor.add_child(prism)
