...
const _WorldObjectScene: PackedScene = preload("res://scenes/world_object.tscn")
...
func _on_structure_placed_visual(wx: int, wz: int, _build_id: StringName, _world_pos: Vector3) -> void:
	refresh_cell(wx, wz)
	_refresh_wood_wall_neighbors(wx, wz)
...
func _on_terrain_edited_visual(wx: int, wz: int, kind: StringName) -> void:
	...
	if k in ["gate", "bridge", "stone_wall", "wood_wall", "dig"]:
		refresh_cell(wx, wz)
		if k in ["wood_wall", "stone_wall", "gate", "bridge", "dig"]:
			_refresh_wood_wall_neighbors(wx, wz)


func _refresh_wood_wall_neighbors(wx: int, wz: int) -> void:
	# Neighbor yaw depends on this cell; rebind so a line/corner orients together.
	refresh_cell(wx - 1, wz)
	refresh_cell(wx + 1, wz)
	refresh_cell(wx, wz - 1)
	refresh_cell(wx, wz + 1)
...
func refresh_cell(wx: int, wz: int) -> void:
	...
	if feat.is_empty():
		return
	if feat.has("build_id") or bool(feat.get("is_passage", false)) or bool(feat.get("is_bridge", false)):
		_StructureOrientation.persist_yaw(wx, wz)
		feat = _FeatureRegistry.get_feature(wx, wz)
...
func _populate_chunk(coord: Vector2i) -> void:
	...
	# Persistent player builds must always come back with the chunk. Billboard
	# budget is only for vegetation / landmarks.
	for x in ChunkData.SIZE:
		for z in ChunkData.SIZE:
			var wx := min_x + x
			var wz := min_z + z
			var feat: Dictionary = _FeatureRegistry.get_feature(wx, wz)
			if feat.is_empty() or not _is_player_built_visual(feat):
				continue
			var anchor := _spawn_feature_anchor(wx, wz, feat)
			if anchor == null:
				continue
	...
	for x in ChunkData.SIZE:
		for z in ChunkData.SIZE:
			...
			var wz := min_z + z
			var feat: Dictionary = _FeatureRegistry.get_feature(wx, wz)
			if feat.is_empty() or _is_player_built_visual(feat):
				continue
			var anchor := _spawn_feature_anchor(wx, wz, feat)
			if anchor == null:
				continue
...
func _is_player_built_visual(feat: Dictionary) -> bool:
	if bool(feat.get("player_built", false)):
		return true
	var build_id := str(feat.get("build_id", "")).strip_edges()
	if build_id in ["wood_wall", "stone_wall", "gate", "bridge"]:
		return true
	return bool(feat.get("is_passage", false)) or bool(feat.get("is_bridge", false))


func _spawn_feature_anchor(wx: int, wz: int, feat: Dictionary) -> Node3D:
	if feat.has("build_id") or bool(feat.get("is_passage", false)) or bool(feat.get("is_bridge", false)):
		_StructureOrientation.persist_yaw(wx, wz)
		feat = _FeatureRegistry.get_feature(wx, wz)
	var anchor := _make_feature_anchor(wx, wz, feat)
...
## Align the authored palisade along the longer neighbor run (X default).
func _wood_wall_yaw(wx: int, wz: int) -> float:
	return _StructureOrientation.yaw_for(wx, wz, "wood_wall")


func _structure_connects(wx: int, wz: int) -> bool:
	var feat: Dictionary = _FeatureRegistry.get_feature(wx, wz)
	if feat.is_empty():
		return false
	var id := _resolve_player_build_visual_id(feat, wx, wz)
	return id in ["wood_wall", "stone_wall", "gate"]


## Align a bridge deck along a neighboring bridge run (X default).
func _bridge_yaw(wx: int, wz: int) -> float:
	return _StructureOrientation.yaw_for(wx, wz, "bridge")


func _is_bridge_cell(wx: int, wz: int) -> bool:
	var feat: Dictionary = _FeatureRegistry.get_feature(wx, wz)
	if feat.is_empty():
		return false
	return _resolve_player_build_visual_id(feat, wx, wz) == "bridge"


func get_anchor_at(wx: int, wz: int) -> Node3D:
	return _nodes_by_cell.get(Vector2i(wx, wz)) as Node3D
...
func _make_building_anchor(
	...
	if tex == null:
		return null
	var feat: Dictionary = _FeatureRegistry.get_feature(wx, wz)
	if _WorldObjectScene != null:
		var obj: Node = _WorldObjectScene.instantiate()
		if obj and obj.has_method("bind"):
			obj.bind(wx, wz, building_id, feat, world, chunk_manager, _crystal, _registry, size, height_offset)
	...
	var yaw: float = float(feat.get("yaw", _StructureOrientation.yaw_for(wx, wz, building_id)))
	mesh_inst.rotation.y = yaw
	return anchor
...
func _player_column_distance(wx: int, wz: int) -> float:
	...
	if player == null:
		return 0.0
	var col := _WorldVisualCoords.column_from_node(player)
	return Vector2(float(wx) + 0.5 - col.x, float(wz) + 0.5 - col.y).length()
