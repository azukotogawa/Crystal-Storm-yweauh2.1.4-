...
func _ready() -> void:
	if _env_flag("CRYSTALSTORM_SAFE_MODE") or _env_flag("CRYSTALSTORM_MINIMAL"):
		apply_safe_mode()
	elif not apply_env_preset_if_set():
		apply_preset(_PerformanceQualityConfig.Preset.MEDIUM)


func apply_env_preset_if_set() -> bool:
...
func _apply_to_scene() -> void:
	...
	_applied = true
	var env_p := OS.get_environment("CRYSTALSTORM_PERF_PRESET").strip_edges().to_lower()
	print("[Perf] Applied preset=%d safe=%s dist=%d crystal=%s flow_cap=%d upload_budget=%dus env=%s" % [
		quality.preset,
		_safe_mode,
		...
		env_p if not env_p.is_empty() else "-",
	])
