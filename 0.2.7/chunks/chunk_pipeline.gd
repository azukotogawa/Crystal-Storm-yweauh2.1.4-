...
static func run_column_stage(
	...
	if data.preserve_column_maps:
		data.preserve_column_maps = false
		data.skip_mesh_plan_cache = true
		if data.has_method("_bind_macro_surface_if_needed"):
			data._bind_macro_surface_if_needed()
		column_source = "resident"
		examined = ChunkData.SIZE * ChunkData.SIZE
		var preserve_us := Time.get_ticks_usec() - t0
		return {
			"examined": examined,
			"column_us": preserve_us,
			"ok": true,
			"stage": STAGE_COLUMN,
...
static func run_mesh_stage(
	...
	var skip_plan: bool = data.skip_mesh_plan_cache
	data.skip_mesh_plan_cache = false
	if full_rebuild and not skip_plan:
...
static func run_worker_job(
	...
	var payload: Dictionary = buf.get("payload", {})
	if payload is Dictionary:
		payload["column_source"] = str(col.get("column_source", ""))
		payload["mesh_source"] = str(mesh.get("mesh_source", ""))
	return {
		"ok": true,
		"payload": payload,
		"mesh_us": mesh_us,
