...
#no longer need weird offset

class WorldDrawer:

    def __init__(self, types, tiles):
        self.CURRENT_TERRAIN_TYPES = types
        self.CURRENT_TERRAIN_TILES = tiles
        ...
        self.terrain_tiles = []
        for terrain_type in self.CURRENT_TERRAIN_TYPES:
            tile_types = []
            for tile_pos in self.CURRENT_TERRAIN_TILES[terrain_type]:
                tile_types.append(self.tilesheet.subsurface((tile_pos[0], tile_pos[1], TILESIZE, TILESIZE)))
            self.terrain_tiles.append(tile_types)

    def draw(self, target_pos, height_map):
            ...
            for x, value in enumerate(row):
                ...
                for terrain_type in self.CURRENT_TERRAIN_TYPES:
                    if terrain_type in tile_corner_types:
                        ...
                        image = self.terrain_tiles[terrain_type][0]
                        break
                self.offset.x = -(self.target_pos[0] - WINDOW_WIDTH / 2)
                self.offset.y = -(self.target_pos[1] - WINDOW_HEIGHT / 2)
                self.display_surface.blit(image, (self.offset.x + (x * TILESIZE), self.offset.y + (y * TILESIZE)))
