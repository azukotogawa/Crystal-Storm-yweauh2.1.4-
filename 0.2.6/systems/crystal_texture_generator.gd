...
func _entity_palette(variant_id: StringName) -> _Palette:
	...
	match str(variant_id):
		"rabbit":
			p.primary = Color(0.88, 0.78, 0.62)
			p.secondary = Color(0.72, 0.58, 0.42)
			p.accent = Color(1.0, 0.92, 0.78)
		"deer":
...
func _vegetation_palette(variant_id: StringName) -> _Palette:
	...
	if id_str.begins_with("bush"):
		p.primary = Color(0.38, 0.68, 0.34)
		p.secondary = Color(0.24, 0.48, 0.22)
		p.accent = Color(0.58, 0.88, 0.48)
	elif id_str.begins_with("tree"):
		p.primary = Color(0.30, 0.58, 0.32)
		p.secondary = Color(0.42, 0.28, 0.16)
		p.accent = Color(0.52, 0.82, 0.42)
	else:
