...
static func build_plant(plant_id: String, stage: int, tint: Color = Color.WHITE) -> Node3D:
	var root := Node3D.new()
	root.name = "VoxelProp"
	var u := unit()
	var growth := float(stage + 1) / 3.0
	var boxes: Array = _plant_boxes(plant_id, u, growth, tint)
	for spec in boxes:
		root.add_child(_box_instance(spec.center, spec.size, spec.color))
	return root
...
static func _plant_boxes(plant_id: String, u: float, growth: float, _tint: Color) -> Array:
	match plant_id:
		"grass_tuft", "grass":
			var h := u * (1.2 + growth * 1.4)
			return [
				{"center": Vector3(-u * 0.35, h * 0.5, 0), "size": Vector3(u * 0.35, h, u * 0.35), "color": Color(0.32, 0.72, 0.28)},
				{"center": Vector3(u * 0.2, h * 0.55, u * 0.15), "size": Vector3(u * 0.32, h * 1.05, u * 0.32), "color": Color(0.42, 0.82, 0.35)},
				{"center": Vector3(0, h * 0.45, -u * 0.25), "size": Vector3(u * 0.3, h * 0.9, u * 0.3), "color": Color(0.28, 0.65, 0.25)},
			]
		"bush":
		...
		_:
			var trunk_h := u * (1.6 + growth * 1.2)
			var canopy_r := u * (1.4 + growth * 1.6)
			return [
				{"center": Vector3(0, trunk_h * 0.5, 0), "size": Vector3(u * 0.7, trunk_h, u * 0.7), "color": Color(0.42, 0.28, 0.18)},
				{"center": Vector3(0, trunk_h + canopy_r * 0.45, 0), "size": Vector3(canopy_r * 1.8, canopy_r * 1.4, canopy_r * 1.8), "color": Color(0.25, 0.55, 0.28)},
				{"center": Vector3(0, trunk_h + canopy_r * 0.95, 0), "size": Vector3(canopy_r * 1.2, canopy_r * 0.9, canopy_r * 1.2), "color": Color(0.35, 0.72, 0.35)},
			]
