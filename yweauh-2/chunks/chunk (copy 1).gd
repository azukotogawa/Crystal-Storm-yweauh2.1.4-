...
# Inside chunk.gd, within generate()
	var current_drift_x = 0
	var current_drift_y = 0
	for y in range(CHUNK_SIZE_y):
		for x in range(CHUNK_SIZE_x):
			var pos = Vector2i(x, y)
	# Use the updated is_disregarded to catch all cliff edges
			if CoordMapper.is_disregarded(Vector2i(cx * CHUNK_SIZE_x + x, cy * CHUNK_SIZE_y + y), world):
				current_drift_x += 1
				current_drift_y += 1
			drift_map[pos] = Vector2i(current_drift_x, current_drift_y)
