class_name CrystalChunkLayer
extends Node3D

var chunk_coord: Vector2i = Vector2i.ZERO
var _mm_instance: MultiMeshInstance3D
var _material: StandardMaterial3D


func setup(coord: Vector2i, material: StandardMaterial3D) -> void:
	chunk_coord = coord
	_material = material
	position = Vector3(
		float(coord.x * ChunkData.SIZE),
		0.0,
		float(coord.y * ChunkData.SIZE)
	)


func rebuild(cells: Array) -> void:
	if _mm_instance == null:
		_mm_instance = MultiMeshInstance3D.new()
		_mm_instance.name = "CrystalTiles"
		add_child(_mm_instance)

	var mm := _mm_instance.multimesh
	if mm == null:
		mm = MultiMesh.new()
		mm.transform_format = MultiMesh.TRANSFORM_3D
		mm.use_custom_data = false
		var box := BoxMesh.new()
		box.size = Vector3.ONE
		mm.mesh = box
		_mm_instance.multimesh = mm

	if _material:
		_mm_instance.material_override = _material

	var count := cells.size()
	mm.instance_count = count
	if count == 0:
		visible = false
		return

	visible = true
	for i in count:
		var cell: CrystalCell = cells[i]
		var y := float(cell.surface_y) + (0.55 if cell.bridge else 0.5)
		var pulse := 0.92 + cell.growth_stage * 0.08
		var basis := Basis.IDENTITY.scaled(Vector3(pulse, pulse * 1.05, pulse))
		var transform := Transform3D(
			basis,
			Vector3(float(cell.world_pos.x) + 0.5, y, float(cell.world_pos.y) + 0.5)
		)
		mm.set_instance_transform(i, transform)