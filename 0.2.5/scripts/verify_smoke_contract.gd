...
func _run() -> void:
	...
	if "data only; visual confirm pending" not in text:
		push_error("smoke dig line must state data-only / visual pending")
		ok = false
	if "visible terrain edit" in text.to_lower():
		push_error("smoke must not claim visible dig without human confirmation")
		ok = false
	if "probe_exit.gd" not in text:
		push_error("smoke must use probe_exit.gd for harness teardown")
		ok = false
