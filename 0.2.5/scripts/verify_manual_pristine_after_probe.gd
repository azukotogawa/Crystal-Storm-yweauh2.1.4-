...
func _run() -> void:
	...
	if "SMOKE GAMEPLAY OK" not in text:
		push_error("smoke wrapper missing OK marker")
		ok = false
	...
	if not FileAccess.file_exists(SMOKE_EVIDENCE):
		...
		ok = false
	elif "data only; visual confirm pending" not in FileAccess.get_file_as_string(SMOKE_EVIDENCE):
		push_error("smoke evidence missing data-only dig label")
		ok = false
	if ok:
