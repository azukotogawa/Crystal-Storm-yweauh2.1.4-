...
func emit_quads():
	...
	for q in quads:
		if int(q.get("face_code", 0)) == FACE_RAMP:
			ramp_quads.append(q)
		else:
			terrain_quads.append(q)

	_emit_box_multimesh(terrain_quads)
	_emit_ramp_multimesh(ramp_quads)
...
func _emit_ramp_multimesh(quads: Array) -> void:
	if quads.is_empty():
		return

	var mm_instance := MultiMeshInstance3D.new()
	mm_instance.name = "ramp_mm_instance"
	layer_container.add_child(mm_instance)
	...
	for i in quads.size():
		...
		var xform := _TerrainRamps.wedge_transform(wx, wz, base_y, dir)
		var b := xform.basis
		var o := xform.origin
		...
		var atlas = VoxelTypes.ATLAS_COORDS.get(q["type"], Vector2i(6, 0))
		var encoded: float = float(FACE_RAMP) + (q["uv_h"] / 100.0)
		buffer[base + 12] = float(atlas.x) / 255.0
		buffer[base + 13] = float(atlas.y) / 255.0
		buffer[base + 14] = encoded
		buffer[base + 15] = q["uv_w"]
